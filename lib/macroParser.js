...
var processObjectArg = function (match, value, index, length) {
  ...
  this.currentMacro.args.push({
    ...
    /* TODOC: 优化传入的数据方式 */
    /*=============================================
    =            将 传入JSON格式字符串 优化成同时兼容 JS对象            =
    =============================================*/

    value: new Function ('return ' + value) (),

    /*=====  End of 将 传入JSON格式字符串 优化成同时兼容 JS对象  ======*/
  });
};
...
var specs = {
    ...
    // TODOC: 新增让undersore定义的宏命令也支持 ejs变量，同时结合es6模板语法来使用
    /*=============================================
    =            优化 让ejs定义的变量也能够 作为引入组件的参数            =
    =============================================*/
    '\`([^\`]*)\`': processStringArg, // 新增

    /*=====  End of 优化 让ejs定义的变量也能够 作为引入组件的参数  ======*/
        "\\s*([\\d|\\.]+)\\s*": processNumArg,
